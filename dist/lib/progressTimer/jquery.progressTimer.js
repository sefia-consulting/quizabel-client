(function ($) {
    $.fn.progressTimer = function (options) {
        ...
        this.each(function () {
            ...
            var warning = (settings.warningThreshold || Math.round(settings.timeLimit*.5)) * 1000;
            var danger = (settings.dangerThreshold || Math.round(settings.timeLimit*.2)) * 1000;
            var interval = window.setInterval(function () {
                var elapsed = new Date() - start;
                bar.width(((elapsed / limit) * 100) + "%");

                if (limit - elapsed <= danger)
                    bar.removeClass(settings.baseStyle)
                       .removeClass(settings.completeStyle)
                       .addClass(settings.dangerStyle);
                else if (limit - elapsed <= warning)
                    bar.removeClass(settings.baseStyle)
                       .removeClass(settings.completeStyle)
    ...
    $.fn.progressTimer.defaults = {
        ...
        warningThreshold: null,  //seconds remaining triggering switch to warning color
        dangerThreshold: null,  //seconds remaining triggering switch to warning color
        ...
        dangerStyle: 'progress-bar-danger',  //bootstrap progress bar style in the warning phase
        completeStyle: 'progress-bar-info'  //bootstrap progress bar style at completion of timer
    };
}(jQuery));
